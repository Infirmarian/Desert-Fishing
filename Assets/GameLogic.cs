...
public class GameLogic : MonoBehaviour {
	...
	void Update () {
		...
		// Only set the exposure if we're not ending.
		if (timeToEnd < 0.0f) {
			if (t < blackoutTime) {
				if (iter == 0) {
		...
		if (timeToEnd < 0.0f) {
			...
			cgs.basic.postExposure = Mathf.Lerp(cgs.basic.postExposure, Mathf.Log(et)/Mathf.Log(2.0f), Time.deltaTime);
			profile.colorGrading.settings = cgs;

			timeToEnd -= Time.deltaTime;

			Debug.Log (timeToEnd);
			Debug.Log (cgs.basic.postExposure);
